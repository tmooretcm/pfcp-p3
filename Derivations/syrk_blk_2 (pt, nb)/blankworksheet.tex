...
\renewcommand{\operation}{ \left[ C \right] := \mbox{\sc syrk\_blk\_var2}( A, C ) }
...
\left(\begin{array}{c c c}
  ...
  = 
  \left(\begin{array}{c c c}
     A_0^TA_0 + C_{00} & A_0^TA_1 + C_{01} & * \\  
     A_1^TA_0 + C_{10} & A_1^TA_1 + C_{11} & * \\  
     C_{20} & C_{21} & C_{22}
  \end{array}\right) 
$}
...
$
  \begin{array}{l}          % do not delete this line 
    C_{10} := A_1^TA_0 + \widehat C_{10}\\
    C_{11} := A_1^TA_1 + \widehat C_{11}\\
    C_{01} := C_{10}^T
